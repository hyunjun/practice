...
class Solution:
    ...
    def isSubsequence(self, s: str, t: str) -> bool:
        if s is None or 0 == len(s):
            return True
        if not (0 < len(s) <= 100) or t is None or not (0 <= len(t) <= 10 ** 4):
            return False
        t = ''.join([c for c in t if c in set(s)])
        board = [[0] * (len(s) + 1) for _ in range(len(t) + 1)]
