...
if __name__ == '__main__':
  max_num = int(urlopen(MAX_ITEM_URL).read())
  print(max_num)
  item_num_string = ''.join([urlopen(url).read().decode('utf8') for url in [TOPSTORIES_URL, NEWSTORIES_URL]])
  ...
  for url in item_urls:
    ...
    if d is None or 'title' not in d or 'url' not in d:
      continue
